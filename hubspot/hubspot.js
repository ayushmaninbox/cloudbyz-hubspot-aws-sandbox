...
async function fetchPosts(industry = '', limit = 100, after = '') {
    ...
    // Filter to exclude AWS blogs (Blog 11, 22, 33, 44, 55, 66)
    const hubspotBlogs = results.filter(post => {
        const name = post.name || '';
        return !/Blog (11|22|33|44|55|66)/.test(name);
    });

    const formatted = hubspotBlogs.map(post => {
        let postTags = [];
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        if (pathname === '/api/blogs' && req.method === 'GET') {
            ...
            const data = await fetchPosts(industry, limit, after);
...
loadAuthorsCache(config.hubspot_token).then(() => {
    server.listen(PORT, () => {
        console.log(`HubSpot SSR backend API running on http://localhost:${PORT}`);
    });
});
